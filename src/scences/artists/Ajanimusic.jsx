...
>
 { `${state?.products[3]?.artistName} `|| Artist}`s  Debut Album {`${state?.products[3]?.name}`|| AlbumName} is out now.
</Typography>
            
   <Button  sx={{"&:hover":{cursor:"pointer",color:"lime", }, marginTop:"10px", color: isDarkMode ? "black" :"white", backgroundColor:shades.secondary[300]}} onClick={() => navigate(`item/${state?.products[3]?._id}`)}>Buy it</Button>
